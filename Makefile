...
lint:  ## run static analysis with flake8
	python -m black --check sec_web_scraper setup.py
	python -m flake8 sec_web_scraper setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black sec_web_scraper/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./sec_web_scraper

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v sec_web_scraper/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v sec_web_scraper/tests --cov=sec_web_scraper --cov-branch --cov-fail-under=75 --cov-report term-missing
